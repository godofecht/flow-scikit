...
// Helper to safely build table rows and avoid XSS.
function appendRowCells(tr, columns) {
  columns.forEach(col => {
...
function buildAccuracyTable() {
  ...
  all.forEach(d => {
    ...
    appendRowCells(tr, [
      ...
      { text: d.dataset },
      { text: d.metric },
      { text: d.sk_score.toFixed(4), class: "num" },
      { text: d.fl_score.toFixed(4), class: "num" },
      { text: `${sign}${diff.toFixed(4)}`, class: `num ${cls}` },
      { text: skFit, class: "num" },
      { text: flFit, class: "num" },
      { text: skPred, class: "num" },
      { text: flPred, class: "num" }
    ]);
...
function buildTimeTable() {
  ...
  all.forEach(d => {
    ...
    appendRowCells(tr, [
      { text: d.algo.replace(/_/g, " ") },
      { text: d.dataset },
      { text: skTotal.toFixed(2), class: "num" },
      { text: flTotal.toFixed(2), class: "num" },
      { text: speedupStr, class: `num ${cls}` }
    ]);
    tbody.appendChild(tr);
  });
}
...
function buildParityTable() {
  ...
  PARITY.forEach(d => {
    ...
    appendRowCells(tr, [
      ...
      { text: matchText, class: matchClass },
      { text: diffStr, class: "num" },
      { text: d.py_ms.toFixed(4), class: "num" },
      { text: d.fl_ms.toFixed(4), class: "num" },
      { text: speedupStr, class: `num ${speedupClass}` }
    ]);
    tbody.appendChild(tr);
  });
}
...
function buildAndroidTable() {
  ...
  all.forEach(d => {
    ...
    appendRowCells(tr, [
      { text: d.algo.replace(/_/g, " ") },
      { text: d.dataset },
      { text: d.score.toFixed(4), class: "num" },
      { text: d.mac_ms != null ? d.mac_ms.toFixed(2) : "-", class: "num" },
      { text: d.and_ms.toFixed(2), class: "num" },
      { text: ratioStr, class: `num ${ratioCls}` }
    ]);
    tbody.appendChild(tr);
  });
}
