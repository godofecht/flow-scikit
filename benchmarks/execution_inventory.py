...
source file, referenced native/numerical symbols and a confidence level.  Rows
...
def safe_file(obj):
    try:
        return inspect.getsourcefile(obj) or inspect.getfile(obj) or ""
    except (OSError, TypeError):
        return ""
    ...
    for name, cls in all_estimators():
        ...
        for op in OPS:
            ...
            yield {
                ...
                "benchmark_coverage": "unknown",
                "parity_coverage": "unknown",
            }
...
def markdown(data):
    ...
    out = ["# sklearn execution-substrate inventory", "", f"Pinned/generated sklearn: `{sklearn.__version__}`", "", "Generated by `benchmarks/execution_inventory.py`; classifications are evidence-backed and conservative.", ""]
    for group in sorted(groups):
        out += [f"## {group}", "", "| Estimator | Operation | Confidence | Evidence |", "|---|---|---|---|"]
        for r in groups[group]:
            ev = "; ".join(r["evidence"][:4]).replace("|", "\\|")
            out.append(f"| `{r['module']}.{r['estimator']}` | `{r['operation']}` | {r['confidence']} | {ev} |")
        out.append("")
    return "\n".join(out)
...
def main():
    ...
    with args.csv.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["sklearn_version","module","estimator","operation","source_entry_point","source_file","execution_class","confidence","flow_scikit_status","benchmark_coverage","parity_coverage","evidence","referenced_symbols"])
        writer.writeheader()
        for r in data:
            q = dict(r); q["evidence"] = json.dumps(q["evidence"]); q["referenced_symbols"] = json.dumps(q["referenced_symbols"]); writer.writerow(q)
